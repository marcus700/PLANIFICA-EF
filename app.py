    ...
    if boton_unidad and problema_u:
        with st.spinner("Escribiendo la unidad oficial..."):
            try:
                ...
                # Se actualizó el modelo a gemini-2.5-flash-latest
                response = client.models.generate_content(model='gemini-2.5-flash-latest', contents=pedido_u, config=types.GenerateContentConfig(system_instruction=instrucciones_u, temperature=0.7))
    ...
    if boton_sesion and tema_s and materiales_s:
        with st.spinner("Escribiendo la sesión oficial..."):
            try:
                ...
                # Se actualizó el modelo a gemini-2.5-flash-latest
                response = client.models.generate_content(model='gemini-2.5-flash-latest', contents=pedido, config=types.GenerateContentConfig(system_instruction=instrucciones, temperature=0.7))
...
with tab3:
    ...
    if boton_rubrica and criterion_r:
        with st.spinner("Escribiendo la rúbrica oficial..."):
            try:
                ...
                # Se actualizó el modelo a gemini-2.5-flash-latest
                response = client.models.generate_content(model='gemini-2.5-flash-latest', contents=pedido_r, config=types.GenerateContentConfig(system_instruction=instrucciones_r, temperature=0.7))
